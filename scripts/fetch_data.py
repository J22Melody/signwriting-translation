...
def parse(raw):
    ...
    is_valid = (raw.startswith('AS') or raw.startswith('M') or raw.startswith('L') or raw.startswith('R') or raw.startswith('B')) \
        and not raw.startswith('LSM') # dirty fix for puddle id 31 "LSM Hands Group 7"
    if not is_valid:
        return False
...
for index, row in enumerate(signbank):
    ...
    # mt-mt: Literature Malta, Literature Malta Archive
    # de-de: Literatur DE (26, not included)
    if puddle_id == 5 or puddle_id == 151 or puddle_id == 152 or \
       puddle_id == 114 or \
       puddle_id == 127 or puddle_id == 147:
        # the first element is the title
        # the second is the main text
        if len(terms) > 1:
            # only take the main text
            if puddle_id == 127 or puddle_id == 147 or puddle_id == 26:
                spoken = terms[-1]
            else:
                spoken = terms[1]
    ...
    # en-sg: Singapore Sign Language (SgSL) Dictionary
    # de-de: Wörterbuch DE
    # de-ch: Wörterbuch CH-de
    # fr-ca: Dictionnaire Quebec
    # fr-be: Dictionnaire BE-fr
    # fr-ch: Dictionnaire CH-fr
    # fr-fr: Dictionnaire FR
    # pt-br: Dicionário Brasil, Enciclopédia Brasil
    # es-es: Diccionario España
    # es-hn: Diccionario Honduras
    # es-ar: Diccionario Argentina
    # es-ni: Diccionario Nicaragua
    # ca-es: Diccionario Catalán
    # ar-tn: Dictionnaire Tunisien (French?)
    # ko-kr: Dictionary Korea
    # mt-mt: Dictionary Malta
    # nl-be: Woordenboek Flanders
    # pl-pl: Słownik PL
    # sk-sk: Slovník CZ
    # sl-sl: Slovar Slovenia
    if puddle_id == 4 or puddle_id == 25 or puddle_id == 28 or \
       ...
       puddle_id == 49 or \
       puddle_id == 58 or \
       puddle_id == 46 or puddle_id == 116 or \
       puddle_id == 55 or \
       puddle_id == 16 or \
       puddle_id == 41 or \
       puddle_id == 67 or \
