...
train_set, val_set, test_set = random_split(dataset, [int(length*0.7), int(length*0.2), int(length*0.1)], generator=torch.Generator().manual_seed(SEED))
...
for row in dataset:
    ...
    if len(row[0]) > max_input_length:
        max_input_length = len(row[0])
    if len(row[1]) > max_output_length:
        max_output_length = len(row[1])
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=max_input_length):
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    for index, training_pair in enumerate(train_set):
        if index == n_iters:
            break
...
# Evaluation
def evaluate(encoder, decoder, sentence, max_length=max_input_length):
    with torch.no_grad():
