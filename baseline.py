...
def trainIters(encoder, decoder, n_iters=None, n_epoch=1, print_every=1, plot_every=10, learning_rate=0.0001):
    start = time.time()
    plot_losses = []
    ...
    for i in range(n_epoch):
        for index, training_pair in enumerate(train_dataloader):
            ...
            if iter % print_every == 0:
                ...
                print_loss_total = 0
                print('%s (%d %.2f%%) %.4f' % (timeSince(start, iter / n_iters),
...
def evaluateRandomly(encoder, decoder, n=10):
    for i in range(n):
        pair = random.choice(list(train_set))
        print('>', pair[0])
        print('=', pair[1])
...
trainIters(encoder1, attn_decoder1, n_iters=None, n_epoch=1, print_every=1, plot_every=10, learning_rate=LEARNING_RATE)
